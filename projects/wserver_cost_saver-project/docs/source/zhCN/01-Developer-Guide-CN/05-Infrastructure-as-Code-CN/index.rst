...
------------------------------------------------------------------------------
所有的 IAC 代码都在 ``wserver_cost_saver/iac/`` 模块下. 下面我们列出了所有与 IAC 相关的重要的文件::
...
------------------------------------------------------------------------------
所有的 CDK Stack 的声明代码都放在了 ``wserver_cost_saver/iac/define/`` 目录下. 其中 ``main.py`` 是 entry point. 由于我们将不同的 AWS 资源分散到了不同的子模块中, 我们可以通过 comment in / out 来控制是部署部分资源还是部署所有资源.

.. dropdown:: wserver_cost_saver/iac/define/main.py
...
.. literalinclude:: ../../../../wserver_cost_saver/iac/exports.py
   :language: python
   :linenos:
...
1. 在 ``wserver_cost_saver/iac/`` 模块中进行开发.
