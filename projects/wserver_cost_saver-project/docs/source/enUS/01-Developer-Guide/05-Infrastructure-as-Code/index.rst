...
The ``cdk`` folder
------------------------------------------------------------------------------
The ``cdk`` folder is the CDK official convention where you should store your IAC declaration code and where you should run the ``cdk deploy`` command. However, for enterprise project, we should break down complicated logics into modules for better maintainability. The ``cdk/app.py`` script just import the declaration code from the ``wserver_cost_saver/iac/`` Python modules.


The ``wserver_cost_saver/iac/`` Python Modules
------------------------------------------------------------------------------
``wserver_cost_saver/iac/`` is the root folder of the infrastructure as code source code. Below are the list of important files related to Infrastructure as Code::

    wserver_cost_saver/iac # the root folder of the infrastructure as code source code
    wserver_cost_saver/iac/define/ # CDK stack declaration code
    wserver_cost_saver/iac/define/main.py # centralized CDK stack object, AWS resources are break down into sub-modules
    wserver_cost_saver/iac/define/iam.py # IAM related AWS resources
    wserver_cost_saver/iac/define/lbd.py # Lambda function related AWS resources
    wserver_cost_saver/iac/exports.py # CDK stack output exports for other projects to use
...
------------------------------------------------------------------------------
1. Working on ``wserver_cost_saver/iac/`` module to declare your CDK stack.
